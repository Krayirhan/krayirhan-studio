...
import { Sparkles, ThumbsUp, Clock, CheckCircle2, Flame } from "lucide-react";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {ROADMAP_ITEMS.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="relative flex flex-col justify-between rounded-3xl border border-slate-800 bg-[#0d1320] p-6 sm:p-8 shadow-xl hover:border-slate-700 transition-all"
            >
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold transition-all cursor-pointer ${
                    voted
                      ...
                      : "border border-slate-700 bg-slate-900 text-slate-300 hover:border-rose-500 hover:text-white"
                  }`}
                >
