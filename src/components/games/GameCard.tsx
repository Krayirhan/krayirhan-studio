...
import { ArrowRight, Download, Gamepad2, Smartphone } from "lucide-react";
    ...
    <div className="group relative flex flex-col justify-between overflow-hidden rounded-[2.2rem] border border-white/10 bg-[var(--card)] p-6 sm:p-7 shadow-xl hover:border-white/25 hover:shadow-2xl hover:shadow-black/70 transition-all duration-300">
      ...
      <div>
        <div className="flex items-center justify-between gap-2 mb-5">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-bold text-zinc-100">
            {product.type === "game" ? (
          ...
          <span className="text-xs font-semibold text-zinc-300 bg-white/5 border border-white/10 px-2.5 py-1 rounded-lg">
            {product.status}
          </span>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap gap-1.5">
            {product.category.slice(0, 3).map((cat) => (
              <span
                key={cat}
                className="rounded-md bg-white/5 border border-white/10 px-2 py-0.5 text-xs font-medium text-zinc-300"
              >
                {cat}
              </span>
            ))}
      ...
      <div className="mt-6 pt-5 border-t border-white/10 flex items-center justify-between gap-3">
        {product.links.playStore ? (
          ...
          >
            ...
            <span className="text-sm font-medium text-zinc-400">Çok Yakında</span>
        )}
