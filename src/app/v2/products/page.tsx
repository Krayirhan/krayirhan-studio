...
export default function V2LegacyProductsPage() {
  const bentoProducts = PRODUCTS.map((product) => ({
    ...
    accent:
      product.id === "blok-dunyasi"
        ? ("orange" as const)
        : product.id === "lingorise"
        ? ("green" as const)
        : ("blue" as const),
    index:
      product.id === "blok-dunyasi"
  ...
  return (
    <div className="v2-page min-h-screen bg-[#080a0d] text-[#f5f2eb] flex flex-col justify-between">
      <V2Header />
      <main className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 space-y-12 flex-1">
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto">
          <div
            className="eyebrow-tag gap-1.5 py-1 pr-3.5 text-xs font-semibold text-zinc-300 mb-4 inline-flex"
            style={{ "--tag-accent": "#a97ee0" } as CSSProperties}
          >
            <span>V2 · Ürün Portföyü (Arşiv)</span>
          </div>
          <h1 className="text-4xl font-extrabold tracking-tight text-white sm:text-5xl">
            Tüm Oyunlar & Uygulamalar
          </h1>
          <p className="mt-4 text-zinc-400 text-base sm:text-lg">
            Krayirhan Studio klasik portföy görünümü.
          </p>
        </div>
